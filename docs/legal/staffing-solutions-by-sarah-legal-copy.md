# Staffing Solutions by Sarah Fell Incorporated — Legal Copy Draft

Status: Drafted based on current setup
Privacy/contact email currently set to: Sarah.fell@staffingsolutionsbysarah.com (confirmed)
Province: Ontario
Current site functions: booking inquiries now; resumes may be collected later
Email marketing/newsletters: not active now

---

## What was corrected

1. The earlier note saying a separate cookie notice is "mandatory under PIPEDA" was too broad.
   - Better: because the site uses analytics, the Privacy Policy must clearly disclose analytics/tracking practices. A banner or consent tool may also be appropriate depending on how the site is configured.

2. The earlier wording around Google Analytics needed tightening.
   - Better: Google Analytics uses cookies and similar technologies to measure site usage.

3. The earlier wording around Vercel needed to be specific to Vercel Web Analytics.
   - Better: Vercel Web Analytics is described by Vercel as using anonymized data and not using cookies.

4. The CASL footer note alone is not enough for compliance.
   - Better: keep a short CASL note, but actual email practices must also respect consent and unsubscribe rules if commercial emails are ever sent.

5. The legal set for this site should be three pages:
   - Privacy Policy
   - Terms of Use
   - Recruitment Disclaimer

---

## Plain-English summary

### Privacy Policy
This tells people what information the business collects, why it collects it, how it uses it, and who to contact about it.

### Terms of Use
This sets the rules for using the website and helps limit business risk if someone relies on site content or misuses the site.

### Recruitment Disclaimer
This says the website does not promise a job, a placement, an interview, or a client engagement. Using the site does not create an employment relationship.

### Analytics
The site tracks usage. That needs to be disclosed clearly.

### CASL
If the business ever starts sending marketing emails or newsletters, it needs proper consent and unsubscribe handling.

---

# PAGE 1 — PRIVACY POLICY

**Privacy Policy**

**Effective Date:** [Insert Date]  
**Last Updated:** [Insert Date]

Staffing Solutions by Sarah Fell Incorporated ("we," "us," "our") respects your privacy and is committed to protecting personal information in accordance with applicable Canadian privacy law, including the Personal Information Protection and Electronic Documents Act (PIPEDA), where applicable.

This Privacy Policy explains how we collect, use, disclose, retain, and protect personal information when you use this website, submit a contact form, book a call, or otherwise interact with us.

## 1. Information We Collect

We may collect personal information that you voluntarily provide, including:
- full name
- email address
- phone number
- company name
- job title or hiring details
- information included in your message or inquiry
- booking details you submit through our scheduling flow

If resume submission or candidate application features are added in the future, we may also collect:
- resume or CV information
- employment history
- qualifications
- other information you choose to submit in connection with a job inquiry or candidate profile

We may also collect limited technical data through analytics tools, such as browser type, device type, pages visited, referral source, and general interaction data.

## 2. Why We Collect Information

We collect and use personal information for purposes reasonably connected to our business, including to:
- respond to inquiries
- schedule calls or meetings
- communicate about recruitment services
- understand hiring needs
- improve website performance and user experience
- maintain internal business records
- comply with legal and regulatory obligations

If candidate submission features are added later, personal information may also be used to review qualifications and assess suitability for current or future recruitment opportunities.

## 3. Consent

By submitting your personal information through this website, you consent to our collection, use, and disclosure of that information for the purposes described in this Privacy Policy.

You may withdraw consent at any time, subject to legal or contractual restrictions and reasonable notice. Withdrawal of consent may limit our ability to respond to your request or provide services.

## 4. Analytics, Cookies, and Tracking

This website may use analytics tools, including Google Analytics and Vercel Web Analytics, to understand site traffic and improve performance.

- **Google Analytics** may use cookies or similar technologies to collect information about website usage and visitor interactions.
- **Vercel Web Analytics** is described by Vercel as using anonymized data and not using cookies.

You can usually manage cookies through your browser settings. Depending on the final technical setup of the site, additional notice or consent tools may also be used.

## 5. Disclosure of Information

We do not sell personal information.

We may disclose personal information only where reasonably necessary, including:
- to service providers that support hosting, scheduling, analytics, or website operations
- where required by law or legal process
- where necessary to protect our rights, safety, systems, or operations

If candidate recruitment services are later added through this site, candidate information may also be shared with potential employer clients where appropriate for recruitment purposes and with the candidate's participation in that process.

## 6. Retention

We retain personal information only for as long as reasonably necessary for the purposes described in this Privacy Policy and to meet legal, regulatory, or legitimate business requirements.

When personal information is no longer required, it will be securely deleted, destroyed, or anonymized where appropriate.

## 7. Safeguards

We use reasonable administrative, technical, and organizational safeguards appropriate to the sensitivity of the information.

No method of transmission over the internet or method of electronic storage is completely secure, and we cannot guarantee absolute security.

## 8. Access and Correction

Subject to applicable law, you may request access to personal information we hold about you and request correction of inaccurate or incomplete information.

## 9. Third-Party Services

This website may link to or use third-party tools or platforms, including scheduling services, analytics tools, or other external services. We are not responsible for the privacy practices, content, or availability of third-party services.

## 10. Future Candidate Submissions

At present, the website is primarily used for inquiries and booking requests.

If resume uploads, candidate profiles, or job application features are added in the future, this Privacy Policy may be updated to describe those collection, use, disclosure, and retention practices in more detail.

## 11. Changes to This Policy

We may update this Privacy Policy from time to time. The updated version will be posted on this page with a revised "Last Updated" date.

## 12. Contact

For privacy questions, access requests, correction requests, or consent withdrawal, contact:

**Staffing Solutions by Sarah Fell Incorporated**  
Sarah.fell@staffingsolutionsbysarah.com  
[Insert Phone Number, if desired]

---

# PAGE 2 — TERMS OF USE

**Terms of Use**

**Effective Date:** [Insert Date]  
**Last Updated:** [Insert Date]

By accessing or using this website, you agree to these Terms of Use. If you do not agree, do not use the website.

## 1. Website Purpose

This website is provided for general information about Staffing Solutions by Sarah Fell Incorporated and its services.

## 2. No Reliance

The content on this website is provided for general informational purposes only. While we aim to keep information reasonably accurate and current, we do not guarantee that all content, service descriptions, availability, or other website information will always be complete, current, or error-free.

## 3. Acceptable Use

You agree not to:
- use the website for unlawful, fraudulent, or harmful purposes
- interfere with the website's operation, availability, or security
- submit false, misleading, or unauthorized information
- attempt unauthorized access to any part of the website or its systems
- copy, scrape, reproduce, or republish site materials without permission except as permitted by law

## 4. Intellectual Property

Unless otherwise stated, all content on this website, including text, branding, graphics, design elements, layout, and other materials, is owned by or licensed to Staffing Solutions by Sarah Fell Incorporated and is protected by applicable intellectual property laws.

## 5. Third-Party Links and Tools

This website may contain links to third-party websites, services, or tools. We do not control and are not responsible for the content, privacy practices, terms, or availability of those third parties.

## 6. No Warranties

This website is provided on an "as is" and "as available" basis. To the fullest extent permitted by law, we disclaim all warranties, representations, and conditions, express or implied, including warranties related to availability, accuracy, fitness for a particular purpose, and non-infringement.

## 7. Limitation of Liability

To the fullest extent permitted by law, Staffing Solutions by Sarah Fell Incorporated will not be liable for any loss, damage, cost, or claim arising from or related to your use of, inability to use, or reliance on this website or its content.

## 8. Changes

We may update these Terms of Use at any time by posting the revised version on this page. Continued use of the website after changes are posted means you accept the updated Terms.

## 9. Governing Law

These Terms of Use are governed by the laws of the Province of Ontario and the laws of Canada applicable therein.

## 10. Contact

Questions about these Terms of Use may be directed to:

**Staffing Solutions by Sarah Fell Incorporated**  
Sarah.fell@staffingsolutionsbysarah.com

---

# PAGE 3 — RECRUITMENT DISCLAIMER

**Recruitment Disclaimer**

**Effective Date:** [Insert Date]  
**Last Updated:** [Insert Date]

The information on this website is provided for general business and recruitment-related purposes only.

## 1. No Guarantee of Placement, Interview, or Hiring

Using this website, submitting a form, or booking a call does not guarantee:
- an interview
- a job offer
- a job placement
- a candidate submission
- a client engagement
- any specific business or recruitment outcome

## 2. No Employment Relationship Created

Visiting this website, sending an inquiry, booking a call, or submitting information through the website does not create an employment relationship, agency relationship, recruiter-client contract, or candidate representation agreement.

## 3. Services and Opportunities May Change

Any services, opportunities, or recruitment-related information described on this website may change, be revised, be paused, or be removed at any time without notice.

If job listings or resume submission features are added later, those postings or opportunities may also change or be removed without notice.

## 4. Independent Decisions

Candidate decisions and hiring decisions remain solely with the individuals, employers, and clients involved. We do not guarantee any particular result.

## 5. No Professional Advice

Nothing on this website constitutes legal advice, employment law advice, HR compliance advice, immigration advice, or other regulated professional advice.

You should obtain your own professional advice where needed.

## 6. Third-Party Services

This website may rely on third-party tools or platforms, including scheduling or analytics providers. We are not responsible for outages, delays, errors, or privacy practices relating to third-party services.

## 7. Contact

Questions about this Disclaimer may be directed to:

**Staffing Solutions by Sarah Fell Incorporated**  
Sarah.fell@staffingsolutionsbysarah.com

---

# Footer microcopy

**Privacy:** Personal information submitted through this site is handled in accordance with our Privacy Policy.

**Analytics:** This site may use analytics tools, including Google Analytics and Vercel Web Analytics, to understand website usage and improve performance.

**CASL:** If you contact us, we may respond to your inquiry. If we ever send commercial electronic messages, we will do so in accordance with applicable consent and unsubscribe requirements under Canada's Anti-Spam Legislation.

**Copyright:** © [Year] Staffing Solutions by Sarah Fell Incorporated. All rights reserved.

---

# Notes for implementation

- Keep the legal content in a readable overlay or full-page panel.
- Use the liquid-glass style for shell, tabs, and controls, not behind long blocks of legal text.
- Use a sticky internal tab navigation: Privacy Policy / Terms of Use / Disclaimer.
- Confirmed privacy/contact email: `Sarah.fell@staffingsolutionsbysarah.com`.
- If resume upload goes live later, revisit the Privacy Policy and expand the candidate-information section before launch.
- If newsletters or automated marketing emails are ever introduced, add explicit consent wording to forms and review CASL compliance in the email flow.
